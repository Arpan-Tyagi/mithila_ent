...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        SELECT price, stock_quantity, min_order_quantity INTO v_price, v_stock, v_moq 
        FROM product_variants 
        WHERE id = v_variant_id FOR UPDATE;
        
        IF v_price IS NULL THEN RAISE EXCEPTION 'Variant % not found', v_variant_id; END IF;
        IF v_moq IS NULL THEN v_moq := 1; END IF;
        IF v_quantity < v_moq THEN RAISE EXCEPTION 'Quantity % below MOQ % for variant %', v_quantity, v_moq, v_variant_id; END IF;
