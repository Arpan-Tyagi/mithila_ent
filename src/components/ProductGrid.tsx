...
import FadeUpBlur from "./FadeUpBlur";
...
const SwatchCard = ({ p, delayOffset }: { p: number, delayOffset: number }) => {
  const [isHovered, setIsHovered] = useState(false);
  const cardRef = useRef(null);

  // Parallax on scroll for each card based on position
  const { scrollYProgress } = useScroll({
    target: cardRef,
    offset: ["start end", "end start"]
  });

  // Create a staggering parallax effect where odd items move differently than even items
  const yParallax = useTransform(scrollYProgress, [0, 1], [50, -50]);

  return (
    <FadeUpBlur inView={true} delay={delayOffset * 0.1}>
      <motion.div
        ref={cardRef}
        style={{ y: yParallax }}
...
export default function ProductGrid({ title }: { title: string }) {
  ...
  return (
    <section className="w-full max-w-7xl mx-auto py-24 px-4 overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {products.map((p, index) => (
          // Notice we pass an offset to the parallax so some move faster/slower
          <div key={p} className={index % 2 === 0 ? "mt-0" : "mt-12"}>
            <SwatchCard p={p} delayOffset={index} />
          </div>
        ))}
      </div>
    </section>
